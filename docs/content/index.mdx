...
const project = new NeonProject({ name: "my-app", region: "aws-us-east-1", org: neon.id });
...
An `Account` is the org/team you provision into; `infra-ts login` authenticates it and `infra-ts link` binds the scope. See [Core concepts](/concepts#accounts).
